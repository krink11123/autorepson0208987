...
def load_config():
    ...
    prompt = prompt.replace("[chashapp]", cashapp).replace("[bitcoin]", bitcoin).replace("[litecoin]", litecoin).replace("[ethereum]", ethereum)
...
async def handle_new_message(event):
    if event.is_private:
        ...
        while True:
            load_config()
            session, already_chatted = get_session(event.sender_id)
